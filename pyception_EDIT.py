...
class MyDlg(wx.Dialog):
    def __init__(self,title='Pyception Lab', pos=None, size=wx.DefaultSize, style=wx.DEFAULT_DIALOG_STYLE|wx.DIALOG_NO_PARENT):
        ...
        self.stimulus_values.SetValue( '0.06, 0.08, 0.1, 0.2, 0.4' ) # catchtrials are added later
...
disptext = "'steallum=%f' % (steallum[stolen],)"
...
while not alldone:
    ...
    if which_expt==exptL:
        #disptext = "'bak=%s stealidx=%d steallum=%f rgb=%s' % ( str(backcol),stolen, steallum[stolen], str(rgb) )" I commented this out - Norick
        disptext = "'steallum=%f' % (steallum[stolen],)"
    else:
        disptext = "'time=%f lum=%f' % ( stolen, steallum[abslumtemp] )"
        # DRC changed this from %f to %d on 9/26/2013
    ...
    if method==methA:
        message.setText("Use up and down arrows until just below threshold.\nThen use 'd' to enter constant stimuli (and respond with YN/2AFC keys above).")
