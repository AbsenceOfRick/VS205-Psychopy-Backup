...
myWin = visual.Window((800,600), fullscr=True, color=numpy.tile(backcon,3) )

fixSpot = visual.PatchStim(myWin,tex="none", mask="gauss", pos=(0.1,0.00), size=(0.05,0.05) )
fixSpot.setColor(dmesgColor, 'rgb255')
fixSpot.setPos( (-0.99, 0) )

#backPed = visual.PatchStim(myWin,tex="none", mask="gauss", pos=(0.0,0.0), size=pedsize, color=pedconf  )
message = visual.PatchStim(myWin,tex="none", mask="circle", pos=(0.0,0.0), size=pedsize, color=pedconf  )

circle = visual.PatchStim(myWin,tex="none", mask="circle", units='pix', pos=(0.0,0.0), size=pedsize, color=pedconf  )

noise = visual.PatchStim( myWin, tex="none", pos=(0.010, -0.01), size=(0.1,0.1) )
...
# message = visual.TextStim(myWin,pos=targpos,
		#alignHoriz='left',height=targheight, ori=00, text='abc', font='Sloan',
...
# r, left, down, up
line1 = visual.ShapeStim( myWin, vertices=((0.35,0), (0.3,0)), closeShape=False, lineRGB=clrFixCross)
line2 = visual.ShapeStim( myWin, vertices=((-0.35,0), (-0.3,0)), closeShape=False, lineRGB=clrFixCross)
line3 = visual.ShapeStim( myWin, vertices=((0,0.35), (0,0.4)), closeShape=False, lineRGB=clrFixCross)
line4 = visual.ShapeStim( myWin, vertices=((0,-0.35), (0,-0.4)), closeShape=False, lineRGB=clrFixCross)
...
pedadj = 80
doDisplay = False
trial_seq[trial]=15 #sets original size of dot
while trial < max_trials: #continue until keypress
	...
	if condition in [2,5,6]:
		...
		circle.setColor(float(trial_seq[trial]), 'rgb255' )
		#circle.setSize(int(pedadj) , units='pix')
		if condition==2:
			circle.setSize( (int(pedadj),int(pedadj) ) )
	...
	# turn off display immediately if they do task
         ...
         if doDisplay:
	      ...
	      msg = ""
	      if condition==2:
	      	msg += 'A=%i pix^2 (diam=%d pixels) \n' % ( int(pedadj/2)**2, int(pedadj) )
              	msg += 'L=%f cd/m^2 (raw=%d) \n' % (interp( calib_steps, calib_vals, val), val  )
              	#msg += 'ped=%i (%f cd/m^2) \n' % (int(pedadj), interp( calib_steps, calib_vals, pedadj) )
	      elif condition==5:
			msg += 't=%i ms\n' % ( int(pedadj) )
			msg += 'L=%f cd/m^2 (raw=%d) \n' % (interp( calib_steps, calib_vals, val), val  )
	      elif condition==6:
			msg += 't=%i ms (betw. flashes)\n' % ( int(pedadj) )
			msg += 'L=%f cd/m^2 (raw=%d) \n' % (interp( calib_steps, calib_vals, val), val  )
	      elif True:
	      ...
	      message2.setText(msg)
              ...
              message2.draw()
				...
				# TODO: Maybe want to draw something per-condition?
              #noise.setSize( (0.5, 0.5) )
              #noise.setTex( 1.0 - numpy.zeros( (4,4) ) )
